...
$time=mysql_real_escape_string($response['run_status']['time_used']);
$memory =mysql_real_escape_string($response['run_status']['memory_used']);
 $slink =  mysql_real_escape_string($response['web_link']);
 

if($response['compile_status']=='OK'){
 $sql=mysql_query(" UPDATE Submissions SET answer='$source',language='$lang' WHERE qid='$qid' AND user_id='$user_id' ");
}
else{
